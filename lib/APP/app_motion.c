...
typedef struct
{
    ...
    IO_staticQueue testQueue;
    IO_staticQueue manualQueue;

    bool moveComplete;
    bool absoluteMode;
    IO_timer_S dwellTimer;
    int32_t stepsPerMM;
...
static void app_motion_private_processInputs()
{
    ...
    const bool stepperStopped = dev_stepper_getState(DEV_STEPPER_CHANNEL_MAIN) == DEV_STEPPER_STATE_STOPPED;
    app_motion_data.inputs.justStopped = (app_motion_data.inputs.stopped == false) & stepperStopped;
    app_motion_data.inputs.stopped = stepperStopped;
}
...
static app_motion_state_E app_motion_private_getDesiredState()
{
    ...
    else
    {
        ...
        case APP_MOTION_WAITING:
            if ((app_motion_data.inputs.motionMode == MODE_MANUAL) && IO_staticQueue_pop(&app_motion_data.manualQueue, &app_motion_data.currentMove))
            {
                desiredState = APP_MOTION_MOVING_MANUAL;
            }
            else if ((app_motion_data.inputs.motionMode == MODE_TEST_RUNNING) && IO_staticQueue_pop(&app_motion_data.testQueue, &app_motion_data.currentMove))
            {
...
static void app_motion_private_exitAction()
{
    ...
    case APP_MOTION_MOVING_MANUAL:
    case APP_MOTION_MOVING_TEST:
        IO_timer_stop(&app_motion_data.dwellTimer);
        break;
    case APP_MOTION_COUNT:
        break;
    default:
...
static void app_motion_private_moveManager_entryAction(void)
{
    ...
    case G4_DWELL:
        IO_timer_init(&app_motion_data.dwellTimer, app_motion_data.currentMove.p);
        IO_timer_start(&app_motion_data.dwellTimer);
    case G28_HOME:
    case G90_ABSOLUTE:
...
static bool app_motion_private_moveManager_runAction(void)
{
    ...
    case G0_RAPID_MOVE:
    case G1_LINEAR_MOVE:
        if (app_motion_data.inputs.justStopped)
        {
            moveComplete = true;
        }
        break;
    ...
    case G3_CCW_ARC_MOVE:
    case G4_DWELL:
        if (IO_timer_expired(&app_motion_data.dwellTimer))
        {
            moveComplete = true;
        }
    case G28_HOME:
...
void app_motion_init(int lock)
{
    ...
    (void)IO_staticQueue_init_lock(&app_motion_data.manualQueue, app_motion_data.manualBuffer, MOTION_MANUAL_BUFFER_SIZE, sizeof(app_motion_move_t), lock);
    (void)IO_staticQueue_init_lock(&app_motion_data.testQueue, app_motion_data.testBuffer, MOTION_TEST_BUFFER_SIZE, sizeof(app_motion_move_t), lock);
}
...
{
    return IO_staticQueue_push(&app_motion_data.testQueue, command);
}

bool app_motion_addManualMove(app_motion_move_t *command)
{
    return IO_staticQueue_push(&app_motion_data.manualQueue, command);
}

void app_motion_clearMoveQueue()
{
    IO_staticQueue_empty(&app_motion_data.testQueue);
    IO_staticQueue_empty(&app_motion_data.manualQueue);
}
