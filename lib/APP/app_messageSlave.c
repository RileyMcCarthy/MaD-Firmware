...
static void app_message_slave_private_runAction()
{
    ...
    case APP_MESSAGE_SLAVE_STATE_READING_DATA:
        if (app_message_slave_data.dataLength > app_message_slave_data.dataIndex)
        {
            ...
            app_message_slave_data.dataIndex += IO_protocol_recieveData(app_message_slave_data.dataRX, remaining);
        }
        break;
    case APP_MESSAGE_SLAVE_STATE_READING_PROCESS:
        break;
...
app_message_slave_awk_E app_message_slave_private_decodeCommand()
{
    ...
    case IO_PROTOCOL_COMMAND_MPROFILE:
        DEBUG_INFO("%s", "Saving new machine profile\n");
    ...
    case IO_PROTOCOL_COMMAND_MOTIONSTATUS:
    {
        ...
        {
            app_message_slave_data.stagedOutput.requestMotionEnabled = false;
        }
        else if (status == MOTIONSTATUS_ENABLED)
        {
            app_message_slave_data.stagedOutput.requestMotionEnabled = false;
        }
        else
        {
            DEBUG_WARNING("%s", "Unknown motion status\n");
        }
...
{
    return app_message_slave_data.output.requestMotionEnabled;
}
