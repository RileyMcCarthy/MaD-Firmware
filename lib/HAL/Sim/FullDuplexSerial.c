...
static void fds_thread(void *arg)
{
    ...
    while (self->isRunning)
    {
        ...
        {
            IO_staticQueue_push(&(self->rx_queue), &rxbyte);
            // printf("Received byte: %d\n", rxbyte);
        }
        if (IO_staticQueue_isempty(&(self->tx_queue)) == false)
        {
            uint8_t byte = 0;
            IO_staticQueue_pop(&(self->tx_queue), &byte);
...
bool fds_start(FullDuplexSerial *self, uint8_t rxpin, uint8_t txpin, uint8_t mode, uint32_t baud)
{
    ...
    if (self != NULL)
    {
        ...
        if (self->socket_id > 0)
        {
            ...
            IO_staticQueue_init(&(self->rx_queue), self->rx_buffer, RX_BUFFER_SIZE, sizeof(uint8_t));
            IO_staticQueue_init(&(self->tx_queue), self->tx_buffer, TX_BUFFER_SIZE, sizeof(uint8_t));
...
{
    uint8_t byte = 0;
    if (IO_staticQueue_pop(&(self->tx_queue), &byte) == false)
    {
        byte = -1;
    }
    return byte;
...
{
    return IO_staticQueue_count(&(self->rx_queue));
}

uint8_t fds_rx(FullDuplexSerial *self)
{
    uint8_t byte = 0;
    while (IO_staticQueue_isempty(&(self->rx_queue)))
    {
        // wait for data
    }
    IO_staticQueue_pop(&(self->rx_queue), &byte);
    return byte;
}
...
void fds_tx(FullDuplexSerial *self, uint8_t byte)
{
    if (self)
    {
        IO_staticQueue_push(&(self->tx_queue), &byte);
    }
    return;
}
...
{
    IO_staticQueue_empty(&(self->rx_queue));
}
