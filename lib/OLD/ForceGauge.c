...
static void continuous_data(void *arg)
{
    ...
    uint32_t transmittx = delay * 35 * 2;
    uint32_t disconnecttx = _clockfreq(); // 1000ms before considered disconnected
    _pinclear(rx);
    ...
    while (1)
    {
        while (!_pinr(rx))
        {
            if ((_cnt() - lastData) > disconnecttx)
            {
                ...
                DEBUG_INFO("force gauge reconnecting after timeout %d/%u\n", (_cnt() - lastData), disconnecttx);
                _pinclear(rx);
