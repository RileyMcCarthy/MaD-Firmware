...
void control_cog_run()
{
    ...
    app_monitor_sample_t prevSample;
    app_monitor_getSample(&prevSample);

    MoveModes currentMoveMode = FUNC_MANUAL_OFF;
    MoveModes lastMoveMode = -1;
    app_motion_move_t manual_move;
    ...
    while (1)
    {
        ...
        else if (forcemN >= 0 && forcemN > machineProfile.performance.maxForceTensile) // Tension
        {
            state_machine_set(PARAM_MOTION_CONDITION, CONDITION_TENSION);
        }
        else if (forcemN < 0 && forcemN < -1 * machineProfile.performance.maxForceCompression) // Compression
        {
        ...
        else if (abs(lastEncoderRead - sample.position) < 100) // STOPPED
        {
        ...
        if (currentMachineState.state == STATE_MOTION) // Motion Enabled
        {
            ...
            {
                // motion_disable(); should make it an outout that is consumed by motion and monitor
            }
            ...
            else
            {
                ...
                if (currentMachineState.motionParameters.mode == MODE_MANUAL)
                {
                    ...
                    /*int max_counter = 0; // navkey_read_max(&navkey);
                    ...
                    else
                    {
                        ...
                        case FUNC_MANUAL_INCREMENTAL_JOG: // Setup the navkey for incremental jog (turn off hold)
                            ...
                            if (navkey.status.LTR) // Left released
                            {
                                ...
                                DEBUG_INFO("DECREASING STEP SIZE, STEP=%d\n", manual_move.x);
                            }
                            if (navkey.status.RTR) // Right released
                            {
                                ...
                                DEBUG_INFO("INCREASING STEP SIZE, STEP=%d\n", manual_move.x);
                            }

                            app_motion_move_t absolute;
                            absolute.g = 91;
                            if (navkey.status.UPR) // Up released
                            {
                                app_motion_move_t manual_move_up;
                                manual_move_up.g = 1;
                                manual_move_up.f = manual_move.f;
                                manual_move_up.x = manual_move.x;
                                app_motion_addManualMove(&absolute);
                                app_motion_addManualMove(&manual_move_up);
                                DEBUG_INFO("FUNCTION=INCREMENTAL_JOG, SPEED=%d, STEP=%d\n", manual_move_up.f, manual_move_up.x);
                            }
                            if (navkey.status.DNR > 0) // Down released
                            {
                                app_motion_move_t manual_move_down;
                                manual_move_down.g = 1;
                                manual_move_down.f = manual_move.f;
                                manual_move_down.x = -1 * manual_move.x;
                                app_motion_addManualMove(&absolute);
                                app_motion_addManualMove(&manual_move_down);
                ...
                else if (currentMachineState.motionParameters.mode == MODE_TEST)
                {
                    ...
                    {
                        app_notification_send(APP_NOTIFICATION_TYPE_SUCCESS, "Now in Test Mode\n");
                    }
        ...
        {
            // motion_disable(); make output to be consumed by motion and monitor
        }

        lastMoveMode = currentMoveMode;
        lastEncoderRead = sample.position;

        memcpy(&lastState, &currentMachineState, sizeof(MachineState));
        memcpy(&sample, &sample, sizeof(app_monitor_sample_t));
        memcpy(&prevSample, &sample, sizeof(app_monitor_sample_t));
    }
}
