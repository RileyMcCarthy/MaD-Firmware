...
char *notification_to_json(app_notification_message_S *notification)
{
    ...
    success &= string_to_json("Type", notification_to_str[notification->type]);
...
char *test_data_to_json(app_monitor_sample_t *data, int count, int index)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        // Add the "Time" field
        success &= int_to_json("Time", data[i].time);

        // Add the "Position" field
        success &= int_to_json("Position", data[i].position);

        // Add the "Velocity" field
        success &= int_to_json("Force", data[i].force);

        // Add the "Acceleration" field
        success &= int_to_json("Setpoint", data[i].setpoint);

        // Add the "Force" field
        success &= int_to_json("Log", data[i].index);
